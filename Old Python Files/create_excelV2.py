...
TOPIC_KEYWORDS = {
    ...
    "Artificial Intelligence": [r'\bartificial intelligence\b', r'\bmachine learning\b', r'\bneural network', r'\bdeep learning\b', r'\b\bai\b'],
    ...
    "Computers/Computing": [r'\bcomput', r'\bprocessor', r'\bsoftware\b', r'\bhardware\b', r'\bdigital\b', r'\bmicroprocessor'],
...
def extract_title(chunk, title_style):
    ...
    if title_style == "first_line":
        ...
        # Return the line immediately after a known section label
...
def extract_body(chunk, title_style):
    """Return everything after the title line as the body."""
    ...
    if title_style == "first_line":
        # Body starts after the first non-blank line
        title_idx = non_blank[0][0]
        return "\n".join(lines[title_idx + 1:]).strip()

    elif title_style == "after_label":
        # Find the label, then skip label + title lines
...
def parse_articles(filepath, source_name, source_num, title_style):
    ...
    for chunk in chunks:
        ...
        body  = extract_body(chunk, title_style)

        row = {
            'Date':       date.strftime('%d %B %Y') if date else None,
            'Year':       year,
            'Nanotech':   count_keyword(chunk, [r'\bnano\w*']),
            'Sources':    source_num,
            'Name':       source_name,
        ...
        for topic, patterns in TOPIC_KEYWORDS.items():
            row[topic] = count_keyword(chunk, patterns)
...
def write_excel(all_rows, output_path):
    columns = (
        ['Date', 'Year', 'Nanotech', 'Sources', 'Name', 'Word count', 'Title', 'Body']
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        # Set column widths — wider for Title and Body
        fixed_widths = {'A': 14, 'B': 6, 'C': 10, 'D': 8, 'E': 18, 'F': 12}
        for col_idx, col_letter in enumerate(
            [chr(65 + i) for i in range(ws.max_column)], start=1
        ):
...
def main():
    ...
    for filename, (source_name, source_num, title_style) in FILE_SOURCE_MAP.items():
        ...
        for i, r in enumerate(rows, 1):
            print(f"  Article {i}: Date={r['Date']}, Nanotech={r['Nanotech']}, Words={r['Word count']}, Title='{r['Title'][:60]}'")
